...
async def on_ready():
	...
	activity = discord.Game(name='Sprouts', type=0)
...
async def todo(message, work: str='Generic', work_time: int=10):
	...
	await message.channel.send('Task Saved')
...
async def battleship(message,ship_size=3, dimension=5):
	...
	while(continue_game==0):
		mygame=""
		await message.channel.send("\n  Guess the coordinates of the hidden ship (eg. A2):  ")

		def check(m):
			return (channel == m.channel) and (author == m.author)
		try:
			game_input = await bot.wait_for('message', check=check, timeout=60)
...
async def help(message):
    help_message='''```Command List\n
        ...
        del/done/rem [task number] - remove the task\n
        start/doing [task number] - start the task\n
        battleship [ship size] [grid size] - play battleship\n```'''
